...
namespace Fantasia.UI
{
    ...
    public class StatusInventoryPanel : MonoBehaviour
    {
        ...
        private static readonly Color BorderColor = UGUIKit.DefaultBorderColor;
        ...
        private void Build()
        {
            ...
            var panelFill = UGUIKit.CreateBorderedPanel(canvasGO.transform, "Panel", Vector2.zero, Vector2.one, BorderColor, new Color(0.96f, 0.96f, 0.96f), 2f);
        ...
        {
            var barFill = UGUIKit.CreateBorderedPanel(parent, "TabBar", new Vector2(0f, 0.92f), new Vector2(1f, 1f), BorderColor, FillColor, 2f);

            _tabButtons = new Button[3];
            for (int i = 0; i < 3; i++)
            {
                float x0 = i * 0.2f + 0.01f;
                var btn = UGUIKit.CreateButton(barFill, $"CharTab{i + 1}", new Vector2(x0, 0.08f), new Vector2(x0 + 0.18f, 0.92f),
            ...
            var closeBtn = UGUIKit.CreateButton(barFill, "CloseButton", new Vector2(0.9f, 0.08f), new Vector2(0.99f, 0.92f),
        ...
        {
            var upper = UGUIKit.CreateRect(parent, "UpperSection", new Vector2(0f, 0.5f), new Vector2(1f, 0.9f));

            var portraitFill = UGUIKit.CreateBorderedPanel(upper, "PortraitPanel", new Vector2(0f, 0f), new Vector2(0.65f, 1f), BorderColor, FillColor, 2f);
            _portraitImage = UGUIKit.CreateImage(portraitFill, "Portrait", new Vector2(0.1f, 0.12f), new Vector2(0.9f, 0.85f), Color.gray);
            _portraitNameText = UGUIKit.CreateText(portraitFill, "Name", new Vector2(0f, 0f), new Vector2(1f, 0.1f), "", 13, TextAnchor.MiddleCenter);

            var statusFill = UGUIKit.CreateBorderedPanel(upper, "StatusPanel", new Vector2(0.68f, 0f), new Vector2(1f, 1f), BorderColor, FillColor, 2f);
            // Two parallel columns (labels / values) instead of one string —
            // keeps numbers lined up regardless of how long each label is.
            _statsLabelText = UGUIKit.CreateText(statusFill, "StatLabels", new Vector2(0.08f, 0.05f), new Vector2(0.62f, 0.95f), "", 11, TextAnchor.UpperLeft);
            _statsValueText = UGUIKit.CreateText(statusFill, "StatValues", new Vector2(0.62f, 0.05f), new Vector2(0.95f, 0.95f), "", 11, TextAnchor.UpperLeft);
        }

        private void BuildInventory(Transform parent)
        {
            var invFill = UGUIKit.CreateBorderedPanel(parent, "InventoryPanel", new Vector2(0f, 0f), new Vector2(1f, 0.48f), BorderColor, FillColor, 2f);

            var gridRect = UGUIKit.CreateRect(invFill, "Grid", new Vector2(0.04f, 0.06f), new Vector2(0.96f, 0.94f));
            ...
            {
                var slotFill = UGUIKit.CreateBorderedPanel(gridRect, $"Slot{i}", Vector2.zero, Vector2.one, BorderColor, SlotFillColor, 2f);

                _slotIcons[i] = UGUIKit.CreateImage(slotFill, "Icon", new Vector2(0.15f, 0.28f), new Vector2(0.85f, 1f), Color.clear);
                _slotLabels[i] = UGUIKit.CreateText(slotFill, "Label", new Vector2(0f, 0f), new Vector2(1f, 0.28f), "", 7, TextAnchor.LowerCenter);
        ...
        {
            var dialogFill = UGUIKit.CreateBorderedPanel(canvasRoot, "DiscardDialog", Vector2.zero, Vector2.one, BorderColor, new Color(0.98f, 0.98f, 0.9f), 2f);
            ...
            _discardDialogLabel = UGUIKit.CreateText(dialogFill, "Message", new Vector2(0.05f, 0.4f), new Vector2(0.95f, 0.95f), "", 11, TextAnchor.MiddleCenter);

            var yesBtn = UGUIKit.CreateButton(dialogFill, "Yes", new Vector2(0.1f, 0.08f), new Vector2(0.48f, 0.35f), "예", new Color(0.85f, 0.9f, 0.85f), 11);
            yesBtn.onClick.AddListener(ConfirmDiscard);

            var noBtn = UGUIKit.CreateButton(dialogFill, "No", new Vector2(0.52f, 0.08f), new Vector2(0.9f, 0.35f), "아니오", new Color(0.9f, 0.85f, 0.85f), 11);
