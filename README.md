...
- **Setup Board Test Scene** — 헥스 보드(`Scripts/Board`) 생성. Play 버튼으로 테스트:
  ...
  - **퀘스트 추적 UI** (화면 우측, 항상 표시 — `Scripts/UI/QuestTrackerPanel.cs`, 참고: `Docs/Concept_Image/Concept/판타지아_UI(1).png`): 메인 1개 + 서브 여러 개 동시 표시 가능, 메인이 항상 맨 위 고정되고 서브는 그 아래로 쌓임. 제목 색으로 메인(금색)/서브(실버) 구분 + 은은한 반짝임 효과. 실제 퀘스트 콘텐츠는 아직 없어 더미 데이터로 UI/메커니즘만 검증하는 단계.
