...
- **Setup Board Test Scene** — 헥스 보드(`Scripts/Board`) 생성. Play 버튼으로 테스트:
  ...
  - **대화창 프로토타입** (`Scripts/UI/DialoguePanel.cs`, `Scripts/UI/DialogueStageController.cs`, 참고: `Docs/Concept_Image/Concept/판타지아_UI(대화).png`): 더미 서브 퀘스트("1차 개발 완성")를 퀘스트 창의 "달성!" 버튼으로 완료하면 전용 **대화 씬(`DialogueTest`)**으로 전환됩니다 — 3D 대화 상대가 있는 공간으로 이동한 뒤(아직 모델이 없어 캡슐로 대체) 화면 하단에 VN 스타일 대화창이 뜨고, 대사마다 화자가 바뀔 수 있습니다(대화창 상단에 화자 이름 표시, 현재 말하는 캐릭터만 밝게 강조하고 나머지는 어둡게). 기본은 화면에 캐릭터 1명만 등장(대부분의 대화가 그렇듯) — 플레이어 자신의 대사("개발자")는 초상화 없이 텍스트로만 표시됩니다. 2명 이상이 동시에 필요한 특정 장면은 그때그때 추가하는 방식(`DialogueStageController`는 캐릭터 수와 무관하게 동작). 우상단 **"스킵" 버튼**은 항상 존재하며 대화를 완전히 닫지 않고 마지막 줄(다음 중요 지점)까지만 건너뜁니다(Ren'Py 등 VN의 "스킵은 다음 선택지 전에 멈춘다" 관례 참고). 마지막 줄이 퀘스트 제안이면 **"수락"/"거절"** 버튼이, 아니면 평범한 **"닫기"** 버튼만 뜹니다(퀘스트 제안이 없는 대화에 수락/거절이 뜨면 어색하다는 판단 — WoW의 "거절 버튼이 없는 강제 퀘스트" 불만 사례 참고). 닫히면 자동으로 `BoardTest`로 복귀합니다. 캐릭터 초상화는 아직 없는 최소 단계(GDD 6.6 참고).
