...
- **Setup Board Test Scene** — 헥스 보드(`Scripts/Board`) 생성. Play 버튼으로 테스트:
  ...
  - **퀘스트 추적 UI** (화면 우측, 항상 표시 — `Scripts/UI/QuestTrackerPanel.cs`, 참고: `Docs/Concept_Image/Concept/판타지아_UI(지형 오브젝트, 퀘스트).png`): 메인 1개 + 서브 여러 개 동시 표시 가능, 메인이 항상 맨 위 고정되고 서브는 그 아래로 쌓임. 제목 색으로 메인(금색)/서브(실버) 구분 + 은은한 반짝임 효과. 실제 퀘스트 콘텐츠는 아직 없어 더미 데이터로 UI/메커니즘만 검증하는 단계.
  ...
  - **대화창 프로토타입** (`Q` 키 — `Scripts/UI/DialoguePanel.cs`, 참고: `Docs/Concept_Image/Concept/판타지아_UI(대화).png`): 더미 서브 퀘스트("1차 개발 완성")를 완료하면 화면 하단에 VN 스타일 대화창이 뜨고, "다음" 버튼으로 4줄을 순서대로 넘기다 마지막에 닫힘. 캐릭터 초상화/스킵/퀘스트 수락 선택지는 아직 없는 최소 단계(GDD 6.6 참고).
